...
export default function CustomizedTables({
  ...
  itemData,
  handleClickRecieved,
  handleClickDelivereded,
  handleClickRejected,
  handleClickUnDoRejected,
}) {
        ...
        <Grid width={500} container spacing={2}>
          ...
          <Grid item xs={8}>
            <Stack direction="row" spacing={2}>
              <Typography>
                {selectedProduct?.isFromBiding ? "Bidding" : "Direct Selling"}
              </Typography>{" "}
            </Stack>
          </Grid>
          <Grid item xs={4}>
          ...
          <Grid item xs={8}>
            <Stack direction="row" spacing={2}>
              <Typography>{selectedProduct?.totalPrice + " LKR"}</Typography>{" "}
              <Chip color="secondary" label={selectedProduct?.paymentStatus} />
            </Stack>
          </Grid>
          <Grid item xs={4}>
            <Typography variant="h6" gutterBottom>
          ...
          <Grid item xs={8}>
            <Typography>
              {selectedProduct?.buyer?.firstName +
                " " +
                selectedProduct?.buyer?.lastName}
            </Typography>
          </Grid>
          <Grid item xs={4}>
            <Typography variant="h6" gutterBottom>
        ...
        <TableBody>
          {itemData?.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <StyledTableCell align="left">
                {row?.paymentStatus == "notpaid" && (
                  <Button
                    onClick={() => {
                      handleClickRecieved(row?.id);
                    }}
                    size="small"
                    variant="outlined"
              ...
              <StyledTableCell align="left">
                <Stack direction="row" spacing={2}>
                  {row?.orderStatus == "place" && (
                    <Button
                      onClick={() => {
                        handleClickDelivereded(row?.id);
                      }}
                      color="info"
                      size="small"
                      variant="outlined"
                  ...
                  {row?.orderStatus == "rejected" && (
                    <Button
                      size="small"
                      onClick={() => {
                        handleClickUnDoRejected(row?.id);
                      }}
                      color="info"
                      variant="outlined"
                      startIcon={<UndoIcon />}
