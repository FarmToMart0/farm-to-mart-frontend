...
const validationSchema = yup.object().shape({
  ...
  quantity: yup.number().required().label('Product Quantity'),
  ...
  const handleChangeDate = (e)=>{
   ...
   useEffect(()=>{
    ...
    if (props.edit==1) {
      ...
      setInitialValues({productName:props.editProduct[0].productName,description:props.editProduct[0].description,quantity:props.editProduct[0].remainQuantity,price:props.editProduct[0].unitPrice,bid:props.editProduct[0].initialBid})
      setValuesArray({productName:props.editProduct[0].productName,description:props.editProduct[0].description,quantity:props.editProduct[0].remainQuantity,price:props.editProduct[0].unitPrice,bid:props.editProduct[0].initialBid})
  ...
  try {
    ...
    if(checked ){
      if (new Date(endDate) < new Date(Date.now())) {
        setErrorMessage({ type: 'error', message:'Past date and time not allowed' });
        setErrorOccured(true);
        setLoadingProductAdd(false)
        return
      }if(values.bid ){
        var [code,res] = await api.farmer.updateProduct({_id:productId,'category':selectCategory,biddingEndin:endDate,'farmer':user.id,'productName':values.productName,'quantity':values.quantity,'unitPrice':values?.price,'initialBid':values?.bid,'description':values.description,'biddingEnable':checked,'paymentOption':selectedPayementOption,'deliveryOption':selectedDeliveryOption,'images':imageList});
      }else{
