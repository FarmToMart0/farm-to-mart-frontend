...
export default function MyCropTable({rows,columns, tab,handleClickEdit,updateHarvestAmount,updateHarvestDate,doSave,doRefresh}) {
  ...
  const handleSubmit = async()=>{
    const result = Joi.validate(harvesteddata,
...
const handleCilickOpenModal =(id)=>{
  ...
  setSelectedProduct(rows.filter((item)=>{if (item.id==id) {
...
const validateProperty = (event) => {
    ...
    <TableContainer component={Paper}>
      ...
      <FormDialog handleClose={handleCloseDialog} openDialog={openDialog1}>
        ...
        <Grid item xs={12}>
              ...
              <FormHelperText id="outlined-weight-helper-text" sx={{ml:'5px', fontSize:'1rem'}}>Harvested Date</FormHelperText>
                <OutlinedInput
                    required
                    id="harvestedDate"
                    name='harvestedDate'
  ...
  <Grid item xs={12}>
        <TextField value={harvesteddata.harvestedAmount} onChange={handleHaversetedData} name='harvestedAmount' id="outlined-basic" label="Amount Of Harvest" variant="outlined" />
        {errors.harvestedAmount && (
                <Alert sx={{ mt: '1vw', mb: '1vw' }} severity="error">{errors.harvestedAmount}</Alert>)}
  </Grid>
  ...
  <Button  onClick={handleSubmit} variant="contained">Save</Button>
        ...
        <ModalBox buttonName={'colse'} handleClickOpen={handleCilickOpenModal} handleClose={handleCloseModal} open={openModal}>
  ...
  <Grid item xs={8}>
  <Stack direction='row' spacing={2}><Typography>{selectedProduct.expectedDate}</Typography> </Stack>
  </Grid>
  <Grid item xs={4}>
  ...
  </Grid>
  {
    !tab &&
    <><Grid item xs={4}>
        <Typography variant="h6" gutterBottom>harvested amount </Typography>
      </Grid>
      <Grid item xs={8}>
        <Typography>{selectedProduct.harvestedAmount}</Typography>
      </Grid>
      <Grid item xs={4}>
        <Typography variant="h6" gutterBottom>harvested Date </Typography>
      </Grid>
      <Grid item xs={8}>
        ...
        <TableBody>
          { tab && rows.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <StyledTableCell align="left"><Button onClick={()=>handleCilickOpenModal(row.id)}  size='small' variant="outlined" startIcon={<RemoveRedEyeIcon/>}>
        View
      </Button></StyledTableCell>
              <StyledTableCell align="left"><Button onClick={()=>handleCilickOpenDialog(row.id)}  size='small' variant="outlined" startIcon={<EditIcon/>}>
        Update Harvest Details
          ...
          { !tab && rows.map((row) => (
            ...
            <StyledTableRow key={row.id}>
              ...
              <StyledTableCell align="left">{!row.isEdit && row.harvestedDate}{row.isEdit &&  <FormControl variant="outlined" sx={{width:'100%'}}>
              ...
              <StyledTableCell align="left">{!row.isEdit && row.harvestedAmount}{row.isEdit &&  <TextField value={parseInt(row.harvestedAmount)} onChange={(event)=>handleEditHaversetedAmount(row.id,event)} name='harvestedAmount' id="outlined-basic" label="Amount Of Harvest" variant="outlined" />
        }{errors.harvestedAmount && (
                <Alert sx={{ mt: '1vw', mb: '1vw' }} severity="error">{errors.harvestedAmount}</Alert>) }</StyledTableCell>
              <StyledTableCell align="left"><Button onClick={()=>handleCilickOpenModal(row.id)}  size='small' variant="outlined" startIcon={<RemoveRedEyeIcon/>}>
        View
      </Button></StyledTableCell>
