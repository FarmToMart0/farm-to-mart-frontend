...
const DataCenter = () => {
  ...
  async function getHarvestDetails(district, crop) {
    try {
      ...
      if (code == 201) {
        ...
        const totalharvest = res.map((item) => {
          return item.totalHarvest;
        });
        const totalexpected = res.map((item) => {
          return item.totalExpected;
        });
        const totalLand = res.map((item) => {
  ...
  useEffect(() => {
    getTopHarvestDetails(
      district ? district : user.district,
      yearForTable ? yearForTable : 2022
    );
  }, [yearForTable]);
  useEffect(() => {
        ...
        <>
          ...
          >
            <SelectingInputField
              ...
              value={district}
              valuesArray={districtList}
            />
            <SelectingInputField
              lable={"Crop Type"}
              handleChangeValue={handleChangeCropType}
              value={cropType}
              valuesArray={cropList}
            />
          </Stack>
          <Grid marginTop={0} container rowSpacing={4.5} columnSpacing={2.75}>
            <Grid
              item
              md={8}
              ...
              >
                <Grid item>
                  <Typography variant="h5">
                    Comparison Of Harvested and Expected Amount of {cropType} in
                    Past Years in {district}
                  </Typography>
                  <Typography variant="caption" display="block" gutterBottom>
                    Yields are measured in Kilogram{" "}
                  </Typography>
                </Grid>
                <Grid item></Grid>
              </Grid>
            ...
            <Grid item xs={12} md={5} lg={4}>
              ...
              >
                <Grid item>
                  <Typography variant="h5">
                    Statistics of Growing Area in {district} district{" "}
                  </Typography>
                  <Typography variant="caption" display="block" gutterBottom>
                    Area ismeasured in hectare{" "}
                  </Typography>
                </Grid>
                <Grid item />
              </Grid>
