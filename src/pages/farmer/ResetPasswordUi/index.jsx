...
export default function ResetPasswordUi() {
  ...
  const handleSubmit = async (event) => {
    ...
    const { error } = result;
    if (!error) {
      await resetPassword({password: reserPassword.password,id:id,token:token,email:email});
    } else {
      const errorData = {};
  ...
  return (
    <ThemeProvider theme={theme}>
      <Container component="main" maxWidth="xs">
       ...
       { !expire && <Box component="form" onSubmit={handleSubmit} noValidate sx={{ mt: 1 }}>
            ...
            {errors.password && (
                  <Alert sx={{ mt: "1vw", mb: "1vw" }} severity="error">
                    ...
                    number, an uppercase letter, a lowercase letter and a
                    special character{" "}
                  </Alert>
                )}
            <TextField
